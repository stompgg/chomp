...
- **Skipped: per-turn active-mon-index cache.** Active mon index changes mid-turn (switch moves call `_setActiveMonIndexPacked`; future Roar-style effects could too). Caching it across a turn breaks correctness. Adjacent-line coalescing (where no state can change between back-to-back `_getActiveMonIndex` reads) is safe but only saves ~3-7k/game — below the keep threshold for the bytecode/complexity cost.
