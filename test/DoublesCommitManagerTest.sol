...
contract DoublesCommitManagerTest is Test {
    ...
    function test_doublesCommitAndReveal() public {
        ...
        bytes32 aliceHash = keccak256(abi.encodePacked(aliceMove0, aliceMove1, salt, aliceExtra0, aliceExtra1));
        ...
        vm.startPrank(ALICE);
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: aliceMove0, extraData0: aliceExtra0, moveIndex1: aliceMove1, extraData1: aliceExtra1, salt: salt}), false);
        vm.stopPrank();
    ...
    function test_doublesExecutionWithAllFourMoves() public {
        ...
        // Verify active mon indices are set correctly for doubles (slot 0)
        ...
        bytes32 bobHash2 = keccak256(abi.encodePacked(bobAttack0, bobAttack1, bobSalt2, bobTarget0, bobTarget1));
        commitManager.commitMove(battleKey, bobHash2);
        vm.stopPrank();

        // Alice reveals first (non-committing player)
        vm.startPrank(ALICE);
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: aliceAttack0, extraData0: aliceTarget0, moveIndex1: aliceAttack1, extraData1: aliceTarget1, salt: salt2}), false);
        vm.stopPrank();

        // Bob reveals
        vm.startPrank(BOB);
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: bobAttack0, extraData0: bobTarget0, moveIndex1: bobAttack1, extraData1: bobTarget1, salt: bobSalt2}), false);
        vm.stopPrank();
    ...
    function test_doublesWrongPreimageReverts() public {
        ...
        vm.startPrank(BOB);
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 0, moveIndex1: SWITCH_MOVE_INDEX, extraData1: 1, salt: bytes32("bobsalt")}), false);
        vm.stopPrank();

        // Alice tries to reveal with wrong moves - should fail
        vm.startPrank(ALICE);
        vm.expectRevert(DefaultCommitManager.WrongPreimage.selector);
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 1, moveIndex1: SWITCH_MOVE_INDEX, extraData1: 0, salt: salt}), false); // Wrong extraData values
        vm.stopPrank();
    }
    ...
    function test_doublesGameOverWhenAllMonsKOed() public {
        ...
        // Turn 1: Both Alice slots target Bob slot 0 (engine currently processes slot 0 targets)
        _doublesCommitRevealExecute(
            battleKey,
            0, 0, 0, 0, // Alice: both slots target Bob slot 0
            0, 0, 0, 0  // Bob: both attack
        );

        // Bob's slot 0 should be KO'd from the massive damage, battle continues
        assertEq(engine.getWinner(battleKey), address(0));
    }
