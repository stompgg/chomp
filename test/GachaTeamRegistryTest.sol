...
contract GachaTeamRegistryTest is Test {
    ...
    // both scaled by the flat GAME_EXP_MULT (2x) that applies to every battle.
    ...
    // The flat GAME_EXP_MULT (2x) applies to every battle regardless of game type, so a CPU
    // battle and a PvP battle award identical exp. (Quest completion stacks on top; tested below.)
    function test_exp_cpuAndPvpEarnSameFlatMult() public {
        _whitelist(CPU);
        _bobOwnsTeam();
        uint256 aliceTeam = _aliceTeamIndex();

        // First (CPU) battle: streak=1, gain = (2+1)*2 = 6.
        _runBattleEnd(_ctxAliceVsCpu(ALICE, 0x0, 0x3, uint16(aliceTeam)));
        assertEq(gachaTeamRegistry.getExp(ALICE, 0), 6, "after CPU win: 6");

        // PvP same day: no streak, same flat 2x. Gain = (2+0)*2 = 4.
    ...
    function test_exp_packing_singleBucket() public {
        ...
        // First-ever battle: streak=1, flat 2x → (2+1)*2 = 6 each.
    ...
    function test_exp_capsAtMax() public {
        ...
        // Each battle awards (2 + streak) * 2 exp to mon 0 (alive). To reach 65535 takes thousands.
    ...
    function test_facets_levelUpsUnlockSequentially() public {
        ...
        // Walk through many battles, each forcing a new-day reset so the streak keeps ratcheting.
        // (2 + streak) * 2 exp per battle; level crossings drive the facet draws. Slow-but-safe.
    ...
    function test_quests_bonusStacksWithDailyMultipliers() public {
        // First-ever PvP battle that also completes the quest. Streak=1, expMult = GAME 2x * Quest 2x = 4.
        gachaTeamRegistry.addQuest(_simpleTurnsQuest(10));
        _bobOwnsTeam();
        uint256 aliceTeam = _aliceTeamIndex();
    ...
    function test_gachaEvent_packsPointsExpFacetsBonusesOutcome() public {
        ...
        // FIRST_ROLL | FIRST_GAME | QUEST (bit 2 is reserved/unused).
    ...
    function test_gachaEvent_lossOutcomeAndNoFirstRollOnSecondBattle() public {
        ...
        assertEq(secondEv.multiplier, 2, "flat game mult, no quest");
    ...
    function test_assignPoints_preservesUpperBits() public {
        ...
        // assignPoints write to the low 128 bits.
        address[] memory toAllow = new address[](1);
        address[] memory empty = new address[](0);
        toAllow[0] = ALICE;
