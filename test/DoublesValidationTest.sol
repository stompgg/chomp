...
contract DoublesValidationTest is Test {
    ...
    function _doublesCommitRevealExecute(
        ...
        if (turnId % 2 == 0) {
            bytes32 aliceHash = keccak256(abi.encodePacked(aliceMove0, aliceMove1, aliceSalt, aliceExtra0, aliceExtra1));
            vm.startPrank(ALICE);
            commitManager.commitMove(battleKey, aliceHash);
            vm.stopPrank();

            vm.startPrank(BOB);
            commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: bobMove0, extraData0: bobExtra0, moveIndex1: bobMove1, extraData1: bobExtra1, salt: bobSalt}), false);
            vm.stopPrank();

            vm.startPrank(ALICE);
            commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: aliceMove0, extraData0: aliceExtra0, moveIndex1: aliceMove1, extraData1: aliceExtra1, salt: aliceSalt}), false);
            vm.stopPrank();
        } else {
            bytes32 bobHash = keccak256(abi.encodePacked(bobMove0, bobMove1, bobSalt, bobExtra0, bobExtra1));
            vm.startPrank(BOB);
            commitManager.commitMove(battleKey, bobHash);
            vm.stopPrank();
    ...
    function test_staminaRegenAffectsBothSlotsInDoubles() public {
        vm.skip(true); // TODO: Enable when Engine supports doubles slot 1
    ...
    function test_onePlayerOneKO_noValidTarget() public {
        ...
        DefaultCommitManager commitManager2 = new DefaultCommitManager(engine);
        ...
        // Turn 0: Initial switch
        {
            ...
            bytes32 aliceSalt = bytes32("alicesalt");
            bytes32 bobSalt = bytes32("bobsalt");
            bytes32 aliceHash = keccak256(abi.encodePacked(SWITCH_MOVE_INDEX, SWITCH_MOVE_INDEX, aliceSalt, uint240(0), uint240(1)));
            vm.startPrank(ALICE);
            commitManager2.commitMove(battleKey, aliceHash);
            vm.stopPrank();
            vm.startPrank(BOB);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 0, moveIndex1: SWITCH_MOVE_INDEX, extraData1: 1, salt: bobSalt}), false);
            vm.stopPrank();
            vm.startPrank(ALICE);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 0, moveIndex1: SWITCH_MOVE_INDEX, extraData1: 1, salt: aliceSalt}), false);
            vm.stopPrank();
            engine.execute(battleKey);
        }
        ...
        // Turn 1: Bob KOs Alice's slot 0
        {
            ...
            bytes32 bobHash = keccak256(abi.encodePacked(uint8(0), uint8(NO_OP_MOVE_INDEX), bobSalt, uint240(0), uint240(0)));
            vm.startPrank(BOB);
            commitManager2.commitMove(battleKey, bobHash);
            vm.stopPrank();
            vm.startPrank(ALICE);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(0), extraData0: 0, moveIndex1: uint8(NO_OP_MOVE_INDEX), extraData1: 0, salt: aliceSalt}), false);
            vm.stopPrank();
            vm.startPrank(BOB);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(0), extraData0: 0, moveIndex1: uint8(NO_OP_MOVE_INDEX), extraData1: 0, salt: bobSalt}), false);
            vm.stopPrank();
            engine.execute(battleKey);
        }
    ...
    function test_bothPlayersOneKO_neitherHasValidTarget() public {
        ...
        // Turn 1: Both slot 1 mons attack opponent's slot 0, KO'ing both
        {
            ...
            bytes32 bobHash = keccak256(abi.encodePacked(uint8(NO_OP_MOVE_INDEX), uint8(0), bobSalt, uint240(0), uint240(0)));
            vm.startPrank(BOB);
            commitManager2.commitMove(battleKey, bobHash);
            vm.stopPrank();
            vm.startPrank(ALICE);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(NO_OP_MOVE_INDEX), extraData0: 0, moveIndex1: uint8(0), extraData1: 0, salt: aliceSalt}), false);
            vm.stopPrank();
            vm.startPrank(BOB);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(NO_OP_MOVE_INDEX), extraData0: 0, moveIndex1: uint8(0), extraData1: 0, salt: bobSalt}), false);
            vm.stopPrank();
            engine.execute(battleKey);
        }
    ...
    function test_revealFailsForInvalidMoveOnKOdSlot() public {
        ...
        vm.startPrank(ALICE);
        vm.expectRevert(abi.encodeWithSelector(DefaultCommitManager.InvalidMove.selector, ALICE));
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(0), extraData0: 0, moveIndex1: uint8(NO_OP_MOVE_INDEX), extraData1: 0, salt: aliceSalt}), false);
        vm.stopPrank();
    }
    ...
    function test_singlePlayerSwitchTurn() public {
        ...
        vm.startPrank(BOB);
        bytes32 bobHash = keccak256(abi.encodePacked(uint8(0), uint8(0), bytes32("bobsalt"), uint240(0), uint240(0)));
        ...
        bytes32 aliceSalt = bytes32("alicesalt");
        vm.startPrank(ALICE);
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 2, moveIndex1: NO_OP_MOVE_INDEX, extraData1: 0, salt: aliceSalt}), true);
        vm.stopPrank();
    ...
    function test_singlePlayerSwitchTurn_withAttack() public {
        ...
        bytes32 aliceSalt = bytes32("alicesalt");
        vm.startPrank(ALICE);
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 2, moveIndex1: 0, extraData1: 1, salt: aliceSalt}), true);
        vm.stopPrank();
    ...
    function test_p1OnlySwitchTurn_slot0KOWithValidTarget() public {
        ...
        vm.startPrank(ALICE);
        bytes32 aliceHash = keccak256(abi.encodePacked(uint8(0), uint8(0), bytes32("alicesalt"), uint240(0), uint240(0)));
        vm.expectRevert(DefaultCommitManager.PlayerNotAllowed.selector);
        commitManager.commitMove(battleKey, aliceHash);
        vm.stopPrank();
        ...
        bytes32 bobSalt = bytes32("bobsalt");
        vm.startPrank(BOB);
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 2, moveIndex1: NO_OP_MOVE_INDEX, extraData1: 0, salt: bobSalt}), true);
        vm.stopPrank();
    ...
    function test_p1OneKO_noValidTarget() public {
        ...
        // Turn 1: Alice KOs Bob's slot 0
        {
            bytes32 aliceSalt = bytes32("as2");
            bytes32 bobSalt = bytes32("bs2");
            bytes32 bobHash = keccak256(abi.encodePacked(uint8(NO_OP_MOVE_INDEX), uint8(NO_OP_MOVE_INDEX), bobSalt, uint240(0), uint240(0)));
            vm.startPrank(BOB);
            commitManager2.commitMove(battleKey, bobHash);
            vm.stopPrank();
            vm.startPrank(ALICE);
            ...
            vm.stopPrank();
            vm.startPrank(BOB);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(NO_OP_MOVE_INDEX), extraData0: 0, moveIndex1: uint8(NO_OP_MOVE_INDEX), extraData1: 0, salt: bobSalt}), false);
            vm.stopPrank();
            engine.execute(battleKey);
        }
    ...
    function test_asymmetric_p0HasTarget_p1NoTarget() public {
        ...
        vm.startPrank(BOB);
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 2, moveIndex1: NO_OP_MOVE_INDEX, extraData1: 0, salt: bytes32("bobsalt")}), true);
        vm.stopPrank();
    ...
    function test_asymmetric_p0NoTarget_p1HasTarget() public {
        ...
        vm.startPrank(ALICE);
        commitManager.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 2, moveIndex1: NO_OP_MOVE_INDEX, extraData1: 0, salt: bytes32("alicesalt")}), true);
        vm.stopPrank();
    ...
    function test_bothSlotsKO_twoReserves() public {
        ...
        DefaultCommitManager commitManager4 = new DefaultCommitManager(engine);
        ...
        // Turn 0: Initial switch
        {
            ...
            bytes32 aliceHash = keccak256(abi.encodePacked(SWITCH_MOVE_INDEX, SWITCH_MOVE_INDEX, aliceSalt, uint240(0), uint240(1)));
            vm.startPrank(ALICE);
            commitManager4.commitMove(battleKey, aliceHash);
            vm.stopPrank();
            vm.startPrank(BOB);
            commitManager4.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 0, moveIndex1: SWITCH_MOVE_INDEX, extraData1: 1, salt: bobSalt}), false);
            vm.stopPrank();
            vm.startPrank(ALICE);
            commitManager4.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 0, moveIndex1: SWITCH_MOVE_INDEX, extraData1: 1, salt: aliceSalt}), false);
            vm.stopPrank();
            engine.execute(battleKey);
        }
        ...
        // Turn 1: Bob KOs both of Alice's active mons
        {
            bytes32 aliceSalt = bytes32("as2");
            bytes32 bobSalt = bytes32("bs2");
            bytes32 bobHash = keccak256(abi.encodePacked(uint8(0), uint8(0), bobSalt, uint240(0), uint240(1)));
            vm.startPrank(BOB);
            commitManager4.commitMove(battleKey, bobHash);
            vm.stopPrank();
            vm.startPrank(ALICE);
            commitManager4.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(NO_OP_MOVE_INDEX), extraData0: 0, moveIndex1: uint8(NO_OP_MOVE_INDEX), extraData1: 0, salt: aliceSalt}), false);
            vm.stopPrank();
            vm.startPrank(BOB);
            commitManager4.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(0), extraData0: 0, moveIndex1: uint8(0), extraData1: 1, salt: bobSalt}), false);
            vm.stopPrank();
            engine.execute(battleKey);
        }
        ...
        vm.startPrank(ALICE);
        commitManager4.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 2, moveIndex1: SWITCH_MOVE_INDEX, extraData1: 3, salt: bytes32("alicesalt3")}), true);
        vm.stopPrank();
    ...
    function test_bothSlotsKO_noReserves_gameOver() public {
        ...
        // Turn 1: Bob KOs both of Alice's mons - game should end
        {
            ...
            vm.stopPrank();
            vm.startPrank(ALICE);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(NO_OP_MOVE_INDEX), extraData0: 0, moveIndex1: uint8(NO_OP_MOVE_INDEX), extraData1: 0, salt: aliceSalt}), false);
            vm.stopPrank();
            vm.startPrank(BOB);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(0), extraData0: 0, moveIndex1: uint8(0), extraData1: 1, salt: bobSalt}), false);
            vm.stopPrank();
            engine.execute(battleKey);
        }
    ...
    function test_continueWithOneMon_afterKONoTarget() public {
        ...
        // Turn 2: Alice attacks with slot 1, Bob attacks
        {
            ...
            bytes32 aliceHash = keccak256(abi.encodePacked(uint8(NO_OP_MOVE_INDEX), uint8(0), aliceSalt, uint240(0), uint240(0)));
            vm.startPrank(ALICE);
            commitManager2.commitMove(battleKey, aliceHash);
            vm.stopPrank();
            vm.startPrank(BOB);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(0), extraData0: 0, moveIndex1: uint8(0), extraData1: 0, salt: bobSalt}), false);
            vm.stopPrank();
            vm.startPrank(ALICE);
            commitManager2.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: uint8(NO_OP_MOVE_INDEX), extraData0: 0, moveIndex1: uint8(0), extraData1: 0, salt: aliceSalt}), false);
            vm.stopPrank();
    ...
    function test_bothSlotsSwitchToSameMon_reverts() public {
        ...
        commitManager4.revealMovePair(battleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 2, moveIndex1: SWITCH_MOVE_INDEX, extraData1: 2, salt: bytes32("alicesalt3")}), true);
        vm.stopPrank();
    }
    ...
    function test_doublesThenSingles_storageReuse() public {
        ...
        vm.startPrank(ALICE);
        commitManager.revealMovePair(doublesBattleKey, RevealedMovesPair({moveIndex0: SWITCH_MOVE_INDEX, extraData0: 2, moveIndex1: NO_OP_MOVE_INDEX, extraData1: 0, salt: bytes32("as")}), true);
        vm.stopPrank();
