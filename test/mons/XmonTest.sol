...
contract XmonTest is Test, BattleHelper {
    ...
    // Regression: a KO delivered by Somniphobia's nightmare tick, fired off the inline round-end
    // stamina regen (INLINE_STAMINA_REGEN_RULESET is production's default path), must still set
    // playerSwitchForTurnFlag for the next turn. _handleMove / _handleEffects recompute the flag via
    // _checkForGameOverOrKO whenever koOccurredFlag is set; _inlineStaminaRegen must do the same,
    // because _inlineRegenStaminaForMon's OnUpdateMonState hook can reach dealDamage (exactly what
    // the nightmare effect does). When the killing tick is the round-end regen -- the last thing in
    // the turn -- nothing downstream observes the KO, so pre-fix the flag stayed at 2 and the engine
    // wrongly ran the next turn as a normal two-sided turn instead of a one-sided forced switch.
    function test_somniphobiaInlineRegenKOSetsForcedSwitchFlag() public {
        Somniphobia somniphobia = new Somniphobia();

        uint32 maxHp = uint32(somniphobia.DAMAGE_DENOM()) * 10; // 80 HP -> 10 dmg/tick at 1 stack

        // Chips Bob down to 5 HP (below one 10-dmg nightmare tick) but does NOT KO on its own,
        // so the killing blow is unambiguously the regen-triggered nightmare tick.
        ...
        StandardAttack staminaBurn = _staminaBurn(4);

        uint256[] memory aliceMoves = new uint256[](2);
        aliceMoves[0] = uint256(uint160(address(somniphobia)));
        aliceMoves[1] = uint256(uint160(address(bigAttack)));

        uint256[] memory bobMoves = new uint256[](1);
        bobMoves[0] = uint256(uint160(address(staminaBurn)));
        ...
        Mon[] memory aliceTeam = new Mon[](1);
        aliceTeam[0] = aliceMon;
        // Bob gets a second mon so the forced switch this bug should trigger is meaningful.
        ...
        // Turn 1: Alice casts Somniphobia; Bob burns 4 stamina so its later rest regens it back.
        // The round-end regen tick already lands one nightmare hit (Bob -> 70 HP).
        _commitRevealExecuteForAliceAndBob(engine, commitManager, battleKey, 0, 0, 0, 0);
        assertEq(
            engine.getMonStateForBattle(battleKey, 1, 0, MonStateIndexName.IsKnockedOut), 0, "Bob alive after turn 1"
        );

        // Turn 2: Alice chips Bob to 5 HP; Bob burns stamina (does NOT rest, so its only regen is
        // the round-end top-up -- the LAST inline-regen call in the turn, after every _handleEffects
        // pass). That regen fires Somniphobia's nightmare tick (10 dmg), the killing blow: a KO that
        // originates inside _inlineStaminaRegen with nothing running afterwards to notice it.
        ...
        assertEq(
            ...
            1,
            "Bob's active mon should be KOed by the regen-triggered nightmare tick"
        );

        // The bug: Bob's active mon just died, so the next turn must be a one-sided forced switch
        // for Bob (playerSwitchForTurnFlag == 1). Because the KO happened inside _inlineStaminaRegen
        // -- whose call sites never re-check koOccurredFlag / call _checkForGameOverOrKO -- the flag
        // is left at 2, so the engine wrongly runs the next turn as a normal both-sides-act turn.
        (, BattleData memory state) = engine.getBattle(battleKey);
        assertEq(state.playerSwitchForTurnFlag, 1, "Bob should be forced to switch after the nightmare KO");
    }
