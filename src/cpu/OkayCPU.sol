...
import {CPUContext, MoveDecision, RevealedMove} from "../Structs.sol";
...
contract OkayCPU is CPU {
        ...
        returns (uint128 moveIndex, uint16 extraData)
    {
        ...
        MoveDecision memory opponentMove = ENGINE.getMoveDecisionForBattleState(battleKey, 0);

        // If it's the first turn, try and find a mon who has a type advantage to the opponent's type1
        if (ctx.turnId == 0) {
            Type opponentType1 =
                Type(ENGINE.getMonValueForBattle(battleKey, 0, uint256(opponentMove.extraData), MonStateIndexName.Type1));
            Type[] memory selfTypes = new Type[](switches.length);
            for (uint256 i = 0; i < switches.length; i++) {
                selfTypes[i] =
                    Type(ENGINE.getMonValueForBattle(battleKey, 1, uint256(switches[i].extraData), MonStateIndexName.Type1));
            }
        ...
        */
        ...
        int32 staminaDelta = ctx.cpuActiveMonStaminaDelta;
        if (staminaDelta <= -3) {
            ...
            int256 hpDelta = ENGINE.getMonStateForBattle(battleKey, 1, activeMonIndex, MonStateIndexName.Hp);
            if (hpDelta != 0) {
                ...
                Type opponentType1 =
                    Type(ENGINE.getMonValueForBattle(battleKey, 0, opponentMonIndex, MonStateIndexName.Type1));
                Type opponentType2 =
                    Type(ENGINE.getMonValueForBattle(battleKey, 0, opponentMonIndex, MonStateIndexName.Type2));
                uint128[] memory physicalOrSpecialMoves =
                    _filterMoves(ctx, moves, MoveClass.Physical, MoveClass.Special);
                if (physicalOrSpecialMoves.length > 0) {
                    uint128[] memory typeAdvantagedMoves = _getTypeAdvantageAttacks(
                        ctx, opponentType1, opponentType2, moves, physicalOrSpecialMoves
                    );
                    if (typeAdvantagedMoves.length > 0) {
                        uint256 rngIndex = _getRNG(battleKey) % typeAdvantagedMoves.length;
                        return (
                            moves[typeAdvantagedMoves[rngIndex]].moveIndex,
                            moves[typeAdvantagedMoves[rngIndex]].extraData
                        );
                    }
                }
            } else {
                uint128[] memory selfOrOtherMoves =
                    _filterMoves(ctx, moves, MoveClass.Self, MoveClass.Other);
    ...
    function _filterMoves(
        ...
        for (uint256 i = 0; i < movesLen; i++) {
            MoveClass currentMoveClass =
                MoveSlotLib.moveClass(ctx.cpuActiveMonMoveSlots[moves[i].moveIndex], ENGINE, ctx.battleKey);
    ...
    function _getTypeAdvantageAttacks(
        ...
        for (uint256 i = 0; i < indicesLen; i++) {
            Type moveType = MoveSlotLib.moveType(
                ctx.cpuActiveMonMoveSlots[attacks[validAttackIndices[i]].moveIndex], ENGINE, ctx.battleKey
            );
    ...
    function _getTypeAdvantageOrNullToDefend(Type attackerType, Type[] memory defenderTypes)
        internal
        view
        returns (int256)
    {
