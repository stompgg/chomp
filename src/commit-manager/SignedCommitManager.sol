...
/// @notice Extends DefaultCommitManager with an optimistic dual-signed commit flow: single-sig
///         per-turn execution (committer = msg.sender, revealer signs), forced single-player moves,
///         and a signature-fallback commit. Per-turn move buffering now lives in the Engine itself
///         (BUILTIN_DUAL_SIGNED_MANAGER flow), so this contract no longer carries a buffer.
contract SignedCommitManager is DefaultCommitManager, EIP712 {
    ...
    /// @notice Thrown when msg.sender is not the committer for this turn
    error NotCommitter();
