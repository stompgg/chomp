...
3. **Codegen** — `TypeScriptCodeGenerator` orchestrates specialized emitters:
   `TypeConverter`, `ExpressionGenerator`, `StatementGenerator`,
...
4. **Runtime replacement check** — files listed in `transpiler-config.json`
   emit a re-export stub instead of transpiled output.
