    ...
    def generate_factories_ts(self) -> str:
        ...
        for name, metadata in sorted(self.contracts.items()):
            if metadata.dependencies:
                params = ', '.join([
                    f'{d.name}: {d.type_name}'
                    for d in metadata.dependencies
                ])
                args = ', '.join([d.name for d in metadata.dependencies])
                lines.append(f'export function create{name}({params}): {name} {{')
        ...
        for name, metadata in sorted(self.contracts.items()):
            if metadata.dependencies:
                ...
                lines.append(f"  container.registerFactory('{name}', [{dep_types}], ({', '.join([d.name for d in metadata.dependencies])}) => new {name}({', '.join([d.name for d in metadata.dependencies])}));")
        lines.append('}')
        lines.append('')
